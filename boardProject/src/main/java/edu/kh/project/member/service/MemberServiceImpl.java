...
public class MemberServiceImpl implements MemberService{
	...
	@Autowired // 등록된 Bean 중에서 같은 타입의 Bean 대입(DI)
	 ...
	 * 
	 * - 아주 옛날 방식 : 데이터 -> 암호화 , 암호화된 데이터 -> 복호화 -> 원본 데이터
	 * 
	 * - 약간 과거 또는 현재 : 데이터 암호화만 가능(SHA) -> 복호화 방법 제공 X
	 * 		-> 마구 잡이로 대입해서 만들어진 암호화 데이터 테이블에 뚫림 
	 * 
	 * - 요즘 많이 사용하는 방식 : BCrypt 암호화 (Spring Security)
	 * 		-> 입력된 문자열(비밀번호)에 Salt를 추가한 후 암호화
	 * 		-> 암호화 할 때마다 결과가 다름
	 * 		-> DB에 입력 받은 비밀번호를 암호화해서 넘겨줘도 비교 불가능
	 * 		-> BCrypt가 함께 제공하는 평문, 암호화 데이터 비교 메서드인 matches()를 이용하면 된다!
	 * 			(같으면 true, 다르면 false)
	 * 
	...
	public Member login(String memberEmail, String memberPw) {
		...
		// 1. memberEmail이 일치하는 회원의 정보를 DB에서 조회(비밀번호 포함!)
