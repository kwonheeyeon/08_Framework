...
@RequestMapping("param") // param으로 시작하는 요청을 해당 Controller에 매핑
public class ParameterController {
	
	// param/main 요청을 처리하는 메서드
	@RequestMapping("main") 
...
//	@RequestMapping(value = "test1", method = RequestMethod.GET)
	...
	public String test1(HttpServletRequest req) {
		...
		// -> Controller 메서드에 작성된 매개 변수 타입에 맞는 객체가
		// 		존재하면 바인딩, 없으면 생성해서 바인딩
	...
	/* 2. @RequestParam 어노테이션 - 낱개(한 개, 단수)개 파라미터 얻어오기
	 * 
	...
	public String test2(
		 ...
		 * 2) log.debug() 또는 lob.info() 등을 이용해서 log 출력
		 * */
	...
	/* 3. @RequestParam 이용하기 - 여러 개(복수, 다수) 파라미터 Controller에서 얻는 방법 */
	...
	// /param/test3 POST 방식 요청을 처리하는 메서드
	@PostMapping("test3")
	public String test3(
	...
	public String test4(
		 ...
		 * 
		 * * defaultValue
		 * 		: required = false이면서, 값이 제출되지 않았을 경우
		 * 			매개변수에 대입할 값 지정
		 */
	...
	public String test5(
		...
		// -> key값과 변수명이 같을 경우
		//		원래는 @RequestParam의 () 생략이 가능했었지만
		//		지금은 생략하면 오류 발생!!!
		
		/* @ModelAttribute */ Member member2 // == 커맨드 객체
		) {
		
		// Member 객체를 생성해서 전달 받은 값 세팅하기
